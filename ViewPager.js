...
var ViewPager = createReactClass({
  ...
  componentWillMount() {
    ...
    var release = (e, gestureState) => {
      ...
      //   gestureState.dx += ((!this.afterFirstMove && this.props.isLoop) ? 1 : 0)
      // }
  ...
  movePage(step, gs, animate = true) {
    ...
    if (animate) {
      this.fling = true;
      this.props.animation(this.state.scrollValue, scrollStep + ((!this.afterFirstMove && this.props.isLoop) ? 1 : 0), gs)
        .start((event) => {
          if (event.finished) {
            postChange();
          }
    ...
    return (
      <View style={{flex: 1}}
        ...
        {this.renderPageIndicator({goToPage: this.goToPage,
                            ...
                            scrollValue: this.afterFirstMove ? this.state.scrollValue : this.state.valueZero,
